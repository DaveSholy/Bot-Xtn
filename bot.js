...
if(message.content === 'قتراح'){//الامر
var shopc = message.guild.channels.find("name","اقتراح")
  if(!shopc) return message.reply("لا يوج روم باسم **اقتراح**")
    let قتراح = '';
...
.addField('     --*v2min-- ' ,' ** لعمل روم صوتي مؤقت** ')
.addField('     --*cc-- ' ,' **لعمل الوان للسيرفر** ')
.addField('     -- -- ' ,' ** أوآمر الإدآرة ** ')
.addField('     --*vkick ** ' ,' ** للطرد  ** ')
.addField('     --*bc ** ' ,' ** للبرودكاست ** ')
.addField('     --*clear** ' , '**لـ مسح الشات** ')
.addField('     --*roles** ' , '**لـ اظهار الرتب الخاصة في السيرفر** ')
.addField('للأستفسار أو الرد على أسئلتكم وأفكـــاركم كلمني على الديسكورد  -- Robert J.Shelby#8899')
.addField('**لدعوة البوت للسيرفر ..**' , '**https://discordapp.com/api/oauth2/authorize?client_id=568279045817499649&permissions=0&scope=bot**')
.setColor('RANDOM')
  message.channel.sendEmbed(embed);
    }
});
...
client.on("message", message => {
    if(message.content.startsWith(prefix + 'v2min')) {
     let args = message.content.split(" ").slice(1);
      ...
      message.channel.send(`:ballot_box_with_check: TemporarySound : \`${nam}\``).then(c => setTimeout(() => c.edit(`<@${message.author.id}> ⏱  انتهى وقت الروم الصوتي`), 120000))  // 120000 دقيقتان
    }
 });
...
client.channels.get('568950882675458048').edit({name : `『 الأعضاء ↩ ${member.guild.memberCount} 』`});
})
